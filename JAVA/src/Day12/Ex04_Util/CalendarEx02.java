...
public class CalendarEx02 {
	...
	public static void main(String[] args) {
		...
		String today = sdf.format(cal.getTime());	// 시간을 추출해오는 getTime()
		System.out.println("현재 시간 : " + today);
		
		// 현재 날짜로부터 N시간 후, N일 후는 언제인지
		// 10분 후
		// cal.add(시간단위 상수, 시간, 날짜/시간 값)
		...
		Calendar lottoCal = Calendar.getInstance();	// 오늘 현재
		// 구입 불가 조건1 : 0~6시
...
//		lottoCal.set(Calendar.HOUR, 9);
		...
		int 요일 = lottoCal.get( Calendar.DAY_OF_WEEK );	// 일~토(1~7)
		...
		String todayOfWeek = "";	// 로또 산 날(오늘)
		
	
		// 조건
		// 일요일 오전 6시 ~ 토요일 오후 8시	: 이번 주 토요일 오후 9시
		switch (요일) {	// 로또 산 날(오늘)의 (요일)
		...
		if(buyYn) {
			...
			System.out.println("추첨일 : " + 추첨일);	// 토요일까지 며칠 남았는지
		}
		else {
