...
public class Ex07_File {
	...
	/**
	 * 폴더를 삭제하는 메소드 // 내가 나를 호출하는 함수를 재귀함수(재귀 : 다시 돌아옴)라고 한다.
	 * @param args
	 */
	private static void deleteDirectory(String filePath) {
		...
		if(fileList != null ) {
			...
			for (File subFile : fileList) {
				// 재귀호출
				deleteDirectory(subFile.getPath());	// 하위 파일경로 전달
			}
		}
	...
	public static void main(String[] args) {
		...
		do {
			...
			switch (menuNo) {
				case 1 :
					// 디렉토리 생성
					makeDirectory(filePath); // 자동으로 메소드 생성
					break;
				case 2 :
					// 디렉토리 파일목록
