...
class Person {
	...
	public String toString() {
		return "StreamArrayList [name=" + name + ", age=" + age + "]";
	}
}
...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		// Q. * 나이가 25살 초과인 사람들만 필터링하여 반복
		list.stream().filter(p -> p.getAge() > 25)
					 .forEach(p -> System.out.println(p));
		System.out.println();
		
		// 정렬 - sorted	(기본은 오름차순)
//		list.stream().sorted(Comparator.comparing(정렬 기준 - 람다식))
		
		// 메소드 레퍼런스 (::)
		// : 클래스명::메소드 명	: 메소드 레퍼런스(참조정보)를 지정하는 문법
...
//		list.stream().sorted(Comparator.comparing(p -> p.getName()))
		...
		long sum = list.stream().filter(p -> p.getName().length() == 2)
								.mapToInt(p -> p.getAge())	// 매핑한 결과를 int 타입으로 반환
								.sum();	// 정수타입으로 변환하여 연산이 돼야 하기 때문
